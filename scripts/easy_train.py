...
if %ERRORLEVEL%==0 (
    ...
    wmic process where processid="%2" call terminate >nul
        ...
        subprocess.Popen(
            [".process_watchdog_helper.bat", str(os.getpid()), str(process.pid)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
def setup_stockfish(directory, repo, branch_or_commit, arch, threads=1):
    ...
    with subprocess.Popen(["make", "build", f"ARCH={arch}", f"-j{threads}"], cwd=srcdir, env=env) as process:
        if process.wait():
    ...
    def get_status_string(self):
        ...
        try:
            if not self._active:
                ...
                lines = [f"Converting network...", f"Run  : {self._current_convert[0]}", f"Epoch: {self._current_convert[1]}"]
                return "\n".join(lines)
...
class TrainerRunsWidget(Widget):
    ...
    def _clear_area(self):
        ...
        for i in range(height):
            self._frame.canvas.print_at(" " * width, self._x + self._offset, self._y + i, colour, attr, background)
    ...
    def update(self, frame_no):
        ...
        for i, run in enumerate(self._runs):
            ...
            if prev_gpu_id != curr_gpu_id:
                ...
                if len(text) < width:
                    text += "-" * (len(text) - width)
                self._frame.canvas.paint(text, self._x + self._offset, self._y + curr_line, colour, attr, background)
                curr_line += 1
            ...
            for line in self._make_run_text(run):
                ...
                self._frame.canvas.paint(line[: width - 1], self._x + self._offset, self._y + curr_line, colour, attr, background)
                curr_line += 1
...
class MainView(Frame):
    ...
    def _update_network_list(self):
        ...
        for i, entry in enumerate(self._network_testing.get_ordered_results()):
            self._networks_view.options.append(
                ([str(i + 1), str(entry.run_id), str(entry.epoch), "", f"{entry.elo:0.1f}", f"±{entry.elo_error:0.1f}"], i)
            )
    ...
    def _quit(self):
        self._scene.add_effect(
            PopUpDialog(self._screen, "Are you sure you want to quit?", ["Yes", "No"], has_shadow=True, on_close=self._quit_on_yes)
        )
...
def app(screen, scene, training_runs, network_testing):
    ...
    try:
        ...
        scenes = [Scene([MainView(screen, training_runs, network_testing)], -1, name="Main")]
...
def parse_cli_args():
    ...
    parser = argparse.ArgumentParser(description="Trains the network.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        "--workspace-path",
    ...
    parser.add_argument("--lr", default=8.75e-4, type=float, metavar="FLOAT", dest="lr", help="Initial learning rate.")
    parser.add_argument(
        "--num-workers",
    ...
    parser.add_argument(
        "--tui", default=True, type=str2bool, metavar="BOOL", dest="tui", help="Whether to show a nice terminal user interface."
    )
    parser.add_argument(
        "--do-network-testing",
        default=True,
    ...
    parser.add_argument(
        "--do-network-training", default=True, type=str2bool, metavar="BOOL", dest="do_network_training", help="Whether to train networks."
    )
    parser.add_argument(
...
def main():
    ...
    if do_network_testing:
        ...
        setup_stockfish(
            stockfish_base_directory, stockfish_base_repo, stockfish_base_branch_or_commit, args.build_engine_arch, args.build_threads
        )
        setup_stockfish(
            stockfish_test_directory, stockfish_test_repo, stockfish_test_branch_or_commit, args.build_engine_arch, args.build_threads
        )
    else:

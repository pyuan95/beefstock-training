...
void feature_transformer_slice_forward(
    ...
    float* shared_output_slice = shared_output + threadIdx.x * {outputs_per_thread};
    for (uint32_t s = 0; s < {outputs_per_thread}; ++s)
    {{
        shared_output_slice[s] = 0.0f;
    }}
    ...
    {{
        ...
        {{
            ...
            {{
                ...
                float* shared_output_slice = shared_output + sq * {outputs_per_square} + offset;
                #pragma unroll
...
void feature_transformer_slice_backward(
    ...
    float* shared_output_grad_slice = shared_output_grad + threadIdx.x * {outputs_per_thread};
    ...
    {{
        ...
        {{
            ...
            {{
                ...
                shared_output_grad_slice = shared_output_grad + sq * {outputs_per_square} + offset;
                #pragma unroll
                for (uint32_t s = 0; s < ({threads_per_square} > 1 ? {outputs_per_thread} : {outputs_per_square}); ++s)
                {{
